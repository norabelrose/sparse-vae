...
class SerializableObject:
    @classmethod
    def from_dict(frozen: dict) -> SerializableObject:
        newObject = cls()
        ...
        for key, value in frozen.items():
            ...
            if hasattr(newObject, key):
                setattr(newObject, key, value)
            else:
                logger = logging.getLogger(__name__)
                logger.warning(f"SerializableObject.from_dict: Ignoring non-existent attribute {key} for {cls.__name__}.")
    ...
    def to_dict() -> dict:
        ...
        for key, value in data.items():
            ...
            if issubclass(value, SerializableObject):
