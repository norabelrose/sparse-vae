    ...
    while True:
        ...
        if not candidates:
            if not polling_msg_shown:
                ...
                print(f"No devices are available with at least {min_free_memory} GB of free memory. "
                      f"Polling every 10 seconds until a suitable GPU is found.")

            sleep(10.0)
        else:
            # After filtering out GPUs that don't have sufficient memory, the "best" one is the one that has the
            # least compute utilization- if there are ties, we use the *amount* of free memory as a tie breaker
            best = min(
                candidates,
                key=lambda handle: (nvmlDeviceGetUtilizationRates(handle).gpu, -nvmlDeviceGetMemoryInfo(handle).free)
            )
