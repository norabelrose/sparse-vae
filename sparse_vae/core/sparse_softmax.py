...
@triton.jit
def _forward(
        X, scale, LUT, RPE, KP_M, ATTN_M, sizemax, stride_zx, stride_zrpe, stride_hrpe, stride_srpe, stride_zkpm,
        stride_zattnm,
        **meta
):
    TN = meta['TN']
    BLOCK = meta['BLOCK']
    pidhm = tl.program_id(0).to(tl.int64)
    pidz = tl.program_id(1).to(tl.int64)
    # create index ranges
    rxm = pidhm % BLOCK
    rbm = pidhm // BLOCK
    ...
    blockid = tl.load(LUT + offset + rbmn * 4 + 0).to(tl.int64)
    columnid = tl.load(LUT + offset + rbmn * 4 + 1).to(tl.int64)
    rowid = tl.load(LUT + offset + rbmn * 4 + 2).to(tl.int64)
    ...
    # apply RPE
    if meta['APPLY_RPE']:
        headid = tl.load(LUT + offset + rbmn * 4 + 3).to(tl.int64)
        prpe = RPE + pidz * stride_zrpe + headid * stride_hrpe + columnid * BLOCK + rowid * BLOCK * stride_srpe + rxm\
               * stride_srpe + rxn
...
def _backward(X, scale, DX, LUT, sizemax, stride_zx, stride_zdx, **meta):
    ...
    blockid = tl.load(LUT + offset + rbmn * 4).to(tl.int64)
...
class _softmax(torch.autograd.Function):
    @staticmethod
    def make_lut(layout, device):
        sizes = torch.tensor([], dtype=torch.int64, device=layout.device)
        # sizes along rows
        ...
        nz = layout.nonzero(as_tuple=False)
        head, rows, columns = nz[:, 0], nz[:, 1], nz[:, 2]
        core = torch.stack((idx, columns, rows, head), dim=1).view(-1)
        # construct look-up table
        offsets = offsets * 4 + 2 * sizes.numel()
        header = torch.stack((sizes, offsets), dim=1).view(-1)
        lut = torch.cat((header, core)).type(torch.int32)

        return lut.to(device), int(sizes.max())

    @staticmethod
    def forward(
            ctx, x, scale, rpe, key_padding_mask, attn_mask, kp_mask_mode, attn_mask_mode, spdims, block, lut, maxlut,
            bench, time
    ):
...
class softmax:
    ...
    def make_lut(self, device):
        ...
        if key not in self.lut_cache:
            self.lut_cache[key] = _softmax.make_lut(self.layout, device)
    ...
    def __call__(
            self, x, scale=1., rpe=None, key_padding_mask=None, attn_mask=None, key_padding_mask_mode='add',
            attn_mask_mode='add'
    ):
        time_y = [None]
        ...
        x = softmax.apply_softmax(
            x, scale, rpe, key_padding_mask, attn_mask, key_padding_mask_mode, attn_mask_mode, self.spdims, self.block,
            lut,
