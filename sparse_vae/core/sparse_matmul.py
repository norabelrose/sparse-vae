...
@triton.jit
def _kernel(
        A, B, C, stride_za, stride_ha, stride_ma, stride_ka, stride_zb, stride_hb, stride_kb, stride_nb, stride_zc,
...
#  MAIN API  #
##############
_LookupTable = namedtuple('_LookupTable', ['lut', 'num_locks', 'width', 'packs'])
class LookupTable(_LookupTable):
    def to(self, device):
        return LookupTable(
            self.lut.to(device) if isinstance(self.lut, torch.Tensor) else [lut.to(device) for lut in self.lut],
            self.num_locks, self.width, self.packs
        )
...
class _matmul(torch.autograd.Function):
    ...
    def get_locks(size, dev):
        if dev not in _matmul.locks or size > _matmul.locks[dev].size(0):
    ...
    @staticmethod
    def make_sdd_lut(layout, block) -> LookupTable:
        start_width = 128 // block
        layout = layout.type(torch.int32)
        superblocks = libtriton.superblock(layout.data_ptr(), *layout.shape, start_width)

        luts, widths, packs = [], [], []
        for size, nnz in superblocks:
            nnz = nnz.reshape(-1, 4)
            width = nnz.shape[0] // (size * size)
            luts.append(torch.from_numpy(nnz).type(torch.int32))
            widths.append(width)
            packs.append(size)

        # create locks
        return LookupTable(lut=luts, num_locks=None, width=widths, packs=packs)

    @staticmethod
    def _sdd_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut: LookupTable):
        # (A * B)^T = (B^T * A^T)
        if trans_c:
            a, b = b, a
        ...
        # create kernel
        total_width = sum([width * pack * pack for width, pack in zip(lut.width, lut.packs)])
        c = torch.zeros((batch_size, total_width, block, block), dtype=dtype, device=device)
        for lut, width, pack in zip(lut.lut, lut.width, lut.packs):
            num_lock = 1
    ...
    @staticmethod
    def make_dxx_lut(layout, block, step, trans, transform=lambda idx: idx) -> LookupTable:
        # load-balancing
        ...
        for z in range(layout.size(0)):
            sizes = torch.sum(layout[z, :, :], 1 if trans else 0)
        ...
        # pointer increments
        nnz = layout.nonzero(as_tuple=False) if trans else layout.transpose(1, 2).nonzero(as_tuple=False)
        ...
        lut = lut.type(torch.int32)
        # create locks
        num_locks = max(1, lockid.max())
        return LookupTable(lut, num_locks, width, packs=None)

    @staticmethod
    def _dds_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut: LookupTable):
        # shapes / dtypes
        AS0 = a.size(0)
        AS1 = a.size(1)
        ...
        locks = _matmul.get_locks(2 * AS0 * AS2 // 32 * lut.num_locks, a.device)
        c = torch.empty((CS0, CS1, CS2, CS3), dtype=dtype, device=a.device)
        grid = lambda meta: [lut.width, triton.cdiv(AS2, meta['TM']), AS0]
        _kernel[grid](
            a,
            b,
            c,
    ...
    @staticmethod
    def _dsd_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut: LookupTable):
        # shapes / dtypes
        ...
        locks = _matmul.get_locks(2 * BS0 * BS3 // 32 * lut.num_locks, a.device)
        c = torch.empty((CS0, CS1, CS2, CS3), dtype=dtype, device=a.device)
        grid = lambda meta: [lut.width, triton.cdiv(BS3, meta['TN']), BS0]
        _kernel[grid](
            a,
            b,
            c,
    ...
    @staticmethod
    def forward(ctx, a, b, trans_a, trans_b, trans_c, mode, spdims, block, c_lut, da_lut, db_lut: LookupTable):
        c = _matmul.fn[mode](a, b, trans_a, trans_b, trans_c, spdims, block, c_lut)
        # save for backward
    ...
    def backward(ctx, dc):
        ...
        if ctx.needs_input_grad[0]:
            ...
            da = _matmul.fn[mode_da](
                dc, b, False, not ctx.trans_b, ctx.trans_a, ctx.spdims, ctx.block, ctx.da_lut
            )
        # gradients w.r.t. b
        if ctx.needs_input_grad[1]:
            mode_db = mode[2] + mode[1] + mode[0]
            db = _matmul.fn[mode_db](
                a, dc, not ctx.trans_a, False, ctx.trans_b, ctx.spdims, ctx.block, ctx.db_lut
            )
...
class matmul:
    def __init__(self, layout, block, mode, trans_a=False, trans_b=False):
        ...
        step = 16
        if self.mode == 'sdd':
            self.c_lut = _matmul.make_sdd_lut(layout, block)
            self.da_lut = _matmul.make_dxx_lut(layout, block, step, True)
            self.db_lut = _matmul.make_dxx_lut(layout, block, step, False)
        elif self.mode == 'dsd':
            self.c_lut = _matmul.make_dxx_lut(layout, block, step, not self.trans_a)
            self.da_lut = _matmul.make_sdd_lut(layout, block)
            self.db_lut = _matmul.make_dxx_lut(layout, block, step, self.trans_a)
        elif self.mode == 'dds':
            self.c_lut = _matmul.make_dxx_lut(layout, block, step, self.trans_b)
            self.da_lut = _matmul.make_dxx_lut(layout, block, step, not self.trans_b)
            self.db_lut = _matmul.make_sdd_lut(layout, block)

        if not mode == 'sdd':
            # Dims to be reduced on the 'inside' of the matmul, either -1 or -2
            trans_dense, trans_sparse, sparse_inner = (trans_b, trans_a, -1) if mode == 'dsd' else (
            trans_a, trans_b, -2)
    ...
    def __call__(self, a, b):
        ...
        # execute
        device = a.device
        luts = self.c_lut.to(device), self.da_lut.to(device), self.db_lut.to(device)
        c = _matmul.apply(a, b, self.trans_a, self.trans_b, False, self.mode, self.spdims, self.block, *luts)
