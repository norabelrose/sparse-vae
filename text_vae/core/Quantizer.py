...
class Quantizer(nn.Module):
    def __init__(self, num_codes: int, code_depth: int, d_model: int = None, num_levels: int = 1, ema_decay: float = 0.99):
    ...
    def forward(self, x: FloatTensor, level: int = 0, quantize: bool = True) -> QuantizerOutput:
        ...
        # Empirically there seems to be a tendency for the norm of the encoder's output to increase early in training.
        # Training can easily diverge at this point with the straight-through gradient estimator becoming very biased.
        ...
        distances = torch.cdist(x.detach(), self.codebook[level])
        output.code_indices = distances.argmin(dim=-1)
        ...
        if self.training:
            ...
            unused_code_distances = code_distances[~used_code_mask.unsqueeze(0).expand_as(code_distances)]
            unused_code_loss = unused_code_distances.mean()

            # Apply exponential moving average update
            if (decay := self.ema_decay) > 0.0:
                observations = x.flatten(end_dim=-2)
                summed_observations = (observations.t() @ flat_onehot).t()

                self.cluster_sizes[level] *= decay
                self.cluster_sizes[level] += (1 - decay) * bincounts.detach()
                self.cluster_sums[level] *= decay
                self.cluster_sums[level] += (1 - decay) * summed_observations.detach()
        ...
        self.used_code_mask[level].index_fill_(0, output.code_indices.flatten(), True)
